...
export default function PipelinePage() {
  ...
  const handleStageUpdate = async (stageNum: number, newStatus: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMsg(`Stage ${stageNum} updated to ${newStatus}`);
        await loadPipeline(selectedApp.id);
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        const d = await res.json();
        setErrorMsg(d.detail || 'Failed to update stage.');
      }
    } catch { setErrorMsg('Connection error.'); }
    finally { setActionLoading(null); }
  };

  const handleAIEvaluate = async (stageNum: number) => {
    if (!selectedApp) return;
    const key = `eval-${stageNum}`;
    setActionLoading(key);
    setErrorMsg('');
    setSuccessMsg('');
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/pipeline/applications/${selectedApp.id}/stages/${stageNum}/evaluate`,
        { method: 'POST', headers: getHeaders() }
      );
      if (res.ok) {
        setSuccessMsg(`Stage ${stageNum} AI Evaluation complete.`);
        await loadPipeline(selectedApp.id);
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        const d = await res.json();
        setErrorMsg(d.detail || 'AI Evaluation failed.');
      }
    } catch { setErrorMsg('Connection error.'); }
    finally { setActionLoading(null); }
  };
  ...
  return (
    <div style={{ display: 'flex', gap: '24px', height: 'calc(100vh - 100px)', minHeight: 0 }}>
      ...
      <div style={{ width: '360px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div>
          <h1 className="text-xl font-bold text-white">Recruitment Pipeline</h1>
          <p className="text-xs text-slate-400 mt-0.5">End-to-End AI recruitment stage tracking</p>
        </div>
      ...
      {/* Right Panel: Detailed 15-Stage Pipeline Tracking */}
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        {!selectedApp ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-white/10 bg-slate-900/20 text-center p-12">
            ...
            <p className="text-sm text-slate-400 max-w-xs">Choose a candidate to view and manage their full 15-stage AI recruitment pipeline.</p>
          </div>
        ) : (
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '8px' }}>
              {pipelineLoading ? (
                ...
                <div className="flex flex-col h-48 items-center justify-center rounded-xl border border-dashed border-white/10 text-center gap-3 p-8">
                  <p className="text-xs text-slate-500">No active stages initialized for this candidate.</p>
                  <button
                    onClick={async () => {
                      const res = await fetch(
                      ...
                      <div key={stage.id} style={{ display: 'flex', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                          ...
                          {idx < pipeline.length - 1 && (
                            <div className={`w-0.5 my-1 min-h-[40px] ${isPassed ? 'bg-emerald-500' : isFailed ? 'bg-rose-500' : 'bg-slate-700'}`} style={{ flex: 1 }} />
                          )}
                        </div>

                        {/* Stage Card */}
                        <div className={`flex-1 mb-3 rounded-xl border p-4 ${borderCls} ${isLocked ? 'opacity-50' : ''} transition-all`}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {(stage.scheduled_at || stage.completed_at) && (
                                <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }} className="text-[10px] text-slate-500">
                                  {stage.scheduled_at && <span>📅 Scheduled: {new Date(stage.scheduled_at).toLocaleString()}</span>}
                                  {stage.completed_at && <span>✅ Completed: {new Date(stage.completed_at).toLocaleString()}</span>}
                                </div>
                              )}
                              {stage.score !== null && (
                              ...
                              {stage.feedback && <p className="mt-1.5 text-xs text-slate-400 leading-relaxed">{stage.feedback}</p>}
                              {stage.ai_recommendation && <p className="mt-1 text-xs text-violet-300">🤖 {stage.ai_recommendation}</p>}
                              {stage.recruiter_feedback && <p className="mt-1 text-xs text-amber-300">👤 Recruiter Note: {stage.recruiter_feedback}</p>}
                            </div>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flexShrink: 0 }}>
                                {!isPassed && (
                                  <button onClick={() => handleStageUpdate(stage.stage_number, 'PASSED')}
                                    ...
                                    className="rounded-lg bg-emerald-600/80 px-2.5 py-1.5 text-[10px] font-bold text-white hover:bg-emerald-500 transition disabled:opacity-50">
                                    {actionLoading === `${stage.stage_number}-PASSED` ? '...' : '✓ Pass Stage'}
                                  </button>
                                )}
                                {!isFailed && (
                                  <button onClick={() => handleStageUpdate(stage.stage_number, 'FAILED')}
                                    disabled={!!actionLoading}
                                    className="rounded-lg bg-rose-600/80 px-2.5 py-1.5 text-[10px] font-bold text-white hover:bg-rose-500 transition disabled:opacity-50">
                                    {actionLoading === `${stage.stage_number}-FAILED` ? '...' : '✗ Fail Stage'}
                                  </button>
                                )}
                                <button onClick={() => handleAIEvaluate(stage.stage_number)}
                                  disabled={!!actionLoading}
                                  className="rounded-lg bg-violet-600/80 px-2.5 py-1.5 text-[10px] font-bold text-white hover:bg-violet-500 transition disabled:opacity-50">
                                  {actionLoading === `eval-${stage.stage_number}` ? '...' : '🤖 AI Evaluate'}
                                </button>
                                <button
                                  onClick={() => {
                                    setEditStage(isEditing ? null : stage.stage_number);
                                    setEditFeedback(stage.recruiter_feedback || '');
                                    setEditScore(stage.score !== null ? String(stage.score) : '');
                                  }}
                                  className="rounded-lg border border-white/10 px-2.5 py-1.5 text-[10px] font-bold text-slate-400 hover:text-white hover:border-white/20 transition">
                                  ✏️ Add Note
                                </button>
                              </div>
                            )}
                          ...
                          {isEditing && (
                            <div className="mt-3 border-t border-white/5 pt-3 space-y-2">
                              ...
                              <textarea placeholder="Write recruiter note or feedback..." value={editFeedback} onChange={e => setEditFeedback(e.target.value)} rows={2}
                              ...
                              <div style={{ display: 'flex', gap: '8px' }}>
                                <button onClick={() => handleStageUpdate(stage.stage_number, stage.status)}
                                  className="flex-1 rounded-lg bg-violet-600 py-1.5 text-xs font-bold text-white hover:bg-violet-500 transition">Save Note</button>
